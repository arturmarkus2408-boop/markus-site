...
- КоАО проиндексирован ЧАСТИЧНО: Общая часть и статьи до 51-1. Если статьи
  КоАО нет в справке — это не значит, что её не существует, значит её ещё
  не выписали. Обязательно отправь проверить на lex.uz.
- Штрафы в КоАО считаются в БРВ и регулярно меняются — суммы не называй.
- По ГК, ГПК, ЭПК, Таможенному и Земельному кодексам, законам о лицензировании,
...
let lawBook = null;          // разобранный справочник
let lawBookTried = false;    // чтобы не дёргать файл при каждом запросе

async function loadLawBook(env, origin) {
  if (lawBook || lawBookTried) return lawBook;
  lawBookTried = true;
  try {
    const resp = await env.ASSETS.fetch(new Request(origin + '/pravo-uz.json'));
...
// Грубая нормализация слова: отрезаем окончание, чтобы «зарплату», «зарплата»
// и «зарплате» совпали. Точная морфология тут не нужна.
function stem(word) {
  const w = word.toLowerCase().replace(/ё/g, 'е');
  // Отрезаем окончание: «пеню» → «пен», «заработной» → «зараб».
  return w.slice(0, Math.min(6, Math.max(3, w.length - 2)));
}

// Клиенты пишут сокращениями. Раскрываем их в слова, которыми названы
// статьи, иначе поиск по названию их не находит.
const SYNONYMS = {
  ...
  мчж: 'юридические лица', ооо: 'юридические лица',
  ...
  эсф: 'счет-фактура', эчф: 'счет-фактура', 'счет-фактура': 'счет-фактура',
  самозанятый: 'индивидуальных предпринимателей и самозанятых',
  штраф: 'финансовые санкции', пеня: 'пеня', декрет: 'беременности и родам',
...
function findArticles(book, text) {
  ...
  const push = (codeKey, code, num) => {
    const art = code.статьи && code.статьи[num];
    if (!art) return;
    const key = codeKey + ':' + num;
    if (seen.has(key)) return;
    seen.add(key);
    found.push({ код: codeKey, кодекс: code, номер: num, ...art });
  };

  const lower = text.toLowerCase();

  // Какой кодекс имеется в виду. Важно: статья 253 есть и в Трудовом
  ...
  // и мог перепутать.
  const hints = {
    ТК: /\bтк\b|трудов|работник|работодател|увольн|отпуск|зарплат|заработн|оклад|кадр|трудоустрой|прогул|совместительств|декрет|больничн/i,
    НК: /\bнк\b|налог|ндс|ндфл|ққс|жшдс|акциз|прибыл|оборот|вычет|деклараци|счет-фактур|счёт-фактур|камеральн|пеня|соли[кқ]/i,
    ООО: /\bооо\b|\bмчж\b|общест|участник|уставн|устав|доля|доли|учредител|дивиденд|наблюдательн|реорганизац|ликвидац|крупн(ая|ые) сделк|аффилирован/i,
  ...
  // на «привет, как дела?» тоже подтягивались бы статьи по случайному
  // совпадению букв.
  const LEGAL_SIGNAL = /стать|кодекс|закон|норматив|\bпкм\b|\bпп\b|\bзру\b|юридич|правов|договор|штраф|санкци|пеня|пеню|пени|\bсуд\b|обязан|ответственност|льгот|отчетност|отчётност|провер|лицензи|регистрац/i;
  if (!hinted.length && !LEGAL_SIGNAL.test(lower)) return [];

  // 1) Номера, названные прямо: «ст. 253», «статья 124», «статьи 333 и 253».
  // Номера с дополнением пишутся через дефис: 121-1, 454-3.
  const numbers = [];
  const re = /(?:ст\.?|стать(?:я|и|е|ю|ей))\s*№?\s*(\d{1,3}(?:\s*-\s*\d)?)/gi;
  let m;
  ...
  for (const w of words) {
    if (SYNONYMS[w]) expanded.push(...SYNONYMS[w].split(' '));
  ...
  if (stems.length) {
    ...
    for (const [codeKey, code] of Object.entries(book.кодексы)) {
      for (const [num, art] of Object.entries(code.статьи || {})) {
        const title = art.название.toLowerCase();
        let hits = 0;
...
function buildLawContext(book, question, history) {
  if (!book) return '';
  // Ищем и по текущему вопросу, и по последней реплике — «а что там про сроки?»
  ...
  out += 'Это точные данные. Они главнее того, что ты помнишь. Номера статей\n';
  out += 'называй только отсюда, названия цитируй дословно.\n';

  for (const [codeKey, list] of Object.entries(byCode)) {
    ...
    for (const a of list) {
      ...
      if (a.текст) out += `    Текст статьи: ${a.текст}\n`;
    }
    out += `  Ссылка: ${code.ссылка}\n`;
  }
  out += '\nЕсли нужной нормы в этой справке нет — не подставляй номер по памяти,\n';
  out += 'а честно скажи, что номер надо проверить на lex.uz.\n\n';
  return out;
}
...
async function handleAssistant(request, env, url) {
  ...
  try {
    ...
    dateContext += buildLawContext(book, question, history);
  } catch {
